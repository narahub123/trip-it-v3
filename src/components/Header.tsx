...
function Header(){
    ...
    return(
            ...
            <div className='header'>
                <div className='header-nav'>
                    <div className='header-nav-left'>
                        <button className={'header-left-button'+(clicked == 0 ? "active" : "")} onClick={()=>{setClicked(0); navigate('/community')}}>
                            홈
                        </button>
                        <button className={'header-left-button'+(clicked == 1 ? "active" : "")} onClick={()=>{setClicked(1); navigate('/planner')}}>
                            플래너
                        </button>
                    </div>
                    {localStorage.key(0) != null ?
                        <div className='header-nav-right'>
                            <div className='header-nav-right-login'>
                                <button className='header-right-button' onClick={()=>{ setClicked(()=>{return 5}); navigate('/mypage') }}>마이페이지</button>
                        ...
                        <div className='header-nav-right'>
                            ...
                            <button className='header-nav-right-write' onClick={()=>{ handleShowAlert();}}>글쓰기</button>
                        </div>
                    }
                </div>
            </div>            
        ...
        {
            write == 1 ? <Write write={write} setWrite={setWrite} Nav={clicked} setNav={setClicked}></Write> : null
        }
        {
            showAlert && <Alert message="로그인 후 이용가능합니다" onClose={handleCloseAlert} />
        }
        </>        
    )
}
