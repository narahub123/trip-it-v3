    ...
    return(
        ...
        <div key={detail.postId} className="detail">
            <div className="detail-img">
                {detail.postPic && (
                  <img 
                  src={detail.postPic.startsWith('/img') ? `${detail.postPic}` : `data:image/${detail.postPic.substring(detail.postPic.lastIndexOf('.') + 1)};base64, ${detail.postPic}`}
                    alt='area_img' 
                        ...
                        <div className="detail-left">
                            ...
                            <div className="detail-left-sub">
                                <div className="detail-left-sub-left">
                                    <p className="detail-left-sub-left-font">조회수 {detail.viewCount}</p>
                                ...
                                <div className="detail-left-sub-right">
                                    {
                                        ...
                                        <>
                                            {detail.exposureStatus == 1 && (
                                                <p className="detail-left-sub-right-font" onClick={() => setRewrite(1)}>수정</p>
                                            )}
                                            <p className="detail-left-sub-right-font" onClick={()=>setDel(1)}>삭제</p><br></br>
                                        </>
                                        : 
                                        <>
                                            <p className="detail-left-sub-right-font" onClick={()=>{setCut(1)}}>차단</p>
                                            <p className="detail-left-sub-right-font" onClick={()=>{setBan(1)}}>신고</p>
                                        </>
                                    }
                                </div>
                            </div>
                                ...
                                <div className="detail-left-plan">
                                    ...
                                    <div className="detail-left-plan-body">
                                        <div className="detail-left-plan-body-box">
                                            ...
                                            <p className="plan-body-box-font">{formatDate(detail.startDate)} - {formatDate(detail.endDate)} ({calculateDuration(detail.startDate, detail.endDate)})</p>
                                        </div>
                                        <div className="detail-left-plan-body-box">
                                            <div>
                                            ...
                                            <p className="plan-body-box-font">{detail.metroName}</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className='detail-divb'>
                                {detail.exposureStatus == 1 ? 
                                    <>
                                    <button className={'detail-button' + (text == '모집완료' ? '2' : '')} disabled={detail.loggedUserId != Number(userId) || text == '모집완료'} onClick={()=>{handleClick()}}>
                                        {text}
                                    </button>
                                    {onclick == true && (
                                        <div className={`list-container ${onclick ? 'open' : ''}`}>
                                        <ul>
                                            <li className='keep'  onClick={()=>{handleClick(); setText('모집중');}}>모집중</li>
                                            <li className='done'  onClick={()=>{handleClicks();}}>모집완료</li>
                                        </ul>
                                        </div>
                                    )}
                                    </>
                                    :
                                    <button className='detail-button2'>
                                        모집완료
                                    </button>
                                }
                            </div>
                            ...
                            <div className='right-box-content-box'>
                                <div className='right-box-content-box-top'>
                                    <div className='box-top-imgfile'>
                                        <img className='dpi' src={detail.userpic || '/img/defaultImage.jpg'}></img>
                                    </div>
                                    <div className='box-top-box'>
                                        <p className='box-top-box-font'>{detail.nickname}</p>
                                        <div className='box-top-box-sub'>
                                            <p className='box-top-box-sub-one'>{getAgeGroup(detail.birth)}</p>
                                            <p className='box-top-box-sub-two'>{changeGender(detail.gender)}</p>
        ...
        {
            rewrite == 1 ? <Rewrite postId={postId} postTitle={details.map(detail => (detail.postTitle))} postContent={details.map(detail => (detail.postContent))} rewrite={rewrite} setRewrite={setRewrite}></Rewrite> : null
        }
        {
            del == 1 ? <Delete postId={postId} del={del} setDel={setDel}></Delete> : null
        }
        {
        ...
        {
            warn == 1? <Warn warn={warn} setWarn={setWarn} onclick={onclick} setOnclick={setOnclick} text={text} setText={setText} postId={postId}></Warn> : null
        }
        </>
    )
}
